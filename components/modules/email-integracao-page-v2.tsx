...
export function EmailIntegracaoPageV2() {
  ...
  async function previewEmails() {
    try {
      setLoading(true)
      const r = await fetch("/api/admin/judicial/email-integracao/teste?action=preview&limit=10", { cache: "no-store" })
      const j = await r.json().catch(() => ({}))
  ...
  async function runTriageNow() {
    try {
      setRunning(true)
      const r = await fetch("/api/admin/judicial/email-integracao/processar?limit=10", { method: "POST", cache: "no-store" })
      const j = await r.json().catch(() => ({}))
