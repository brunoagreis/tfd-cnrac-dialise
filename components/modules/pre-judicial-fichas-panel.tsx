...
export function PreJudicialFichasPanel({ caseId }: { caseId: string }) {
  const { user } = useAuth()
  ...
  async function loadCase() {
    try {
      setLoading(true)
      const response = await fetch(`/api/pre-judicial/casos/${encodeURIComponent(caseId)}`, {
        cache: "no-store",
      })
      const data = await response.json().catch(() => ({}))

      if (!response.ok || !data?.ok || !data?.item) {
        throw new Error(data?.error || "Processo pré-judicial não encontrado.")
      }

      setCaseItem({
          ...(data.item || {}),
          fichas: Array.isArray(data.item?.fichas) ? data.item.fichas : [],
          procedures: Array.isArray(data.item?.procedures) ? data.item.procedures : [],
          cids: Array.isArray(data.item?.cids) ? data.item.cids : [],
        } as JudicialCase)
    } catch (error) {
      console.error("LOAD_PRE_JUDICIAL_FICHAS_CASE_ERROR", error)
  ...
  async function handleUploadFicha() {
    ...
    try {
      ...
      form.append("module", "pre-judicial")
  ...
  async function handleSaveFicha() {
    ...
    try {
      ...
      const response = await fetch(`/api/pre-judicial/casos/${encodeURIComponent(caseId)}/fichas`, {
        method: "POST",
      ...
      console.error("SAVE_PRE_JUDICIAL_FICHA_ERROR", error)
  ...
  async function updateFichaAction(action: string, fichaId: string, extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      console.error("UPDATE_PRE_JUDICIAL_FICHA_ERROR", error)
  ...
  async function deleteFicha(item: JudicialFicha) {
    ...
    try {
      ...
      console.error("DELETE_PRE_JUDICIAL_FICHA_ERROR", error)
  ...
  if (!caseItem) {
    return <p className="py-8 text-center text-sm text-muted-foreground">Processo pré-judicial não encontrado.</p>
  }
