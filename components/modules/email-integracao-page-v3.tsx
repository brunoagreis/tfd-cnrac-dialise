...
type PreviewItem = { uid: string; messageId?: string; subject: string; from: string; date: string; classifier: string; pgeNet: string; processo: string; detectedIn: string; bodyText?: string; attachments: FileItem[]; simulatedAction?: { label?: string } }
...
export function EmailIntegracaoPageV3() {
  ...
  useEffect(() => { void loadUsers(); void loadHistory(1) }, [])
  ...
  async function previewEmails() {
    try {
      ...
      setRouting((current) => {
        const next = { ...current }
        for (const item of items) if (!next[String(item.uid)]) next[String(item.uid)] = { modulo: inferModule(item), responsavelId: "" }
        return next
      })
  ...
  function setRoute(uid: string, patch: Partial<{ modulo: string; responsavelId: string }>) {
    setRouting((current) => ({ ...current, [uid]: { modulo: current[uid]?.modulo || "judicial", responsavelId: current[uid]?.responsavelId || "", ...patch } }))
  }
  ...
  async function createServiceOrder(item: PreviewItem) {
    ...
    const r = await fetch("/api/email-os", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ uid: item.uid, messageId: item.messageId, subject: item.subject, from: item.from, date: item.date, classifier: item.classifier, pgeNet: item.pgeNet, processo: item.processo, detectedIn: item.detectedIn, bodyText: item.bodyText || "", modulo: selected.modulo, responsavelId: selected.responsavelId, attachments }),
    })
    const j = await r.json().catch(() => ({}))
    if (!r.ok || !j?.ok) {
      setPreview((current) => [item, ...current])
      return toast.error(j?.error || "Erro ao criar ordem de serviço.")
    }
  ...
  return <div className="flex flex-col gap-6">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div><h1 className="text-2xl font-bold tracking-tight">Integração de e-mail</h1><p className="text-sm text-muted-foreground">Leia a caixa, visualize anexos, processe e direcione mensagens.</p></div>
