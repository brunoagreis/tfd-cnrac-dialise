...
const id = (p: string) => `${p}${randomUUID().replace(/-/g, "")}`
const norm = (v: unknown) => text(v).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")

function stripHtml(v: unknown) {
  return text(v).replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<br\s*\/?\s*>/gi, "\n").replace(/<\/p>/gi, "\n").replace(/<[^>]+>/g, " ").replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/\s+/g, " ").trim()
}
...
function safeName(v: string) {
  return text(v).normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 170) || "arquivo"
}
async function processed(messageId: string) {
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string; status: string | null }>>(
    `SELECT id::text AS id, status FROM public.judicial_email_processados WHERE message_id = $1 LIMIT 1`,
    messageId,
  )
  return rows[0] || null
}
async function findMatch(number: string): Promise<Match | null> {
  ...
  if (!n) return null
  const d = digits(n)
  const linked = await prisma.$queryRawUnsafe<Match[]>(`
    SELECT b.id::text AS "monitoramentoId", COALESCE(b.demanda_id::text, d.id::text, '') AS "demandaId", COALESCE(d.protocolo, b.ficha_core, b.id::text) AS protocolo, COALESCE(p.nome, b.nome_paciente, '') AS "pacienteNome"
    ...
    WHERE COALESCE(pv.ativo, TRUE) = TRUE AND (pv.numero ILIKE $1 OR regexp_replace(COALESCE(pv.numero, ''), '\\D', '', 'g') = $2)
    ORDER BY b.id DESC LIMIT 1
  `, `%${n}%`, d).catch(() => [] as Match[])
  if (linked[0]) return linked[0]
  const rows = await prisma.$queryRawUnsafe<Match[]>(`
    SELECT b.id::text AS "monitoramentoId", d.id::text AS "demandaId", d.protocolo, COALESCE(p.nome, b.nome_paciente, '') AS "pacienteNome"
    FROM public.demandas d
    LEFT JOIN public.judicial_monitoramento_base b ON b.demanda_id = d.id
    LEFT JOIN public.pacientes p ON p.id = d."pacienteId"
    WHERE d.protocolo ILIKE $1 OR d."observacoesUnidade" ILIKE $1 OR regexp_replace(COALESCE(d."observacoesUnidade", ''), '\\D', '', 'g') LIKE $2
    ORDER BY d."createdAt" DESC LIMIT 1
  `, `%${n}%`, `%${d}%`).catch(() => [] as Match[])
  return rows[0] || null
}
async function matchRule(subject: string, body: string, classifier: string): Promise<Rule> {
  const rules = await prisma.$queryRawUnsafe<Array<{ id: string; nome: string; palavras: unknown }>>(`SELECT id::text AS id, nome, palavras_chave AS palavras FROM public.judicial_email_regras WHERE ativo = TRUE ORDER BY ordem ASC, nome ASC`)
  const hay = norm(`${subject} ${body} ${classifier}`)
  for (const r of rules) {
    const words = Array.isArray(r.palavras) ? r.palavras.map(text).filter(Boolean) : []
    if (words.some((w) => hay.includes(norm(w)))) {
      const users = await prisma.$queryRawUnsafe<Array<{ id: string; nome: string; email: string }>>(`SELECT usuario_id AS id, usuario_nome AS nome, COALESCE(usuario_email, '') AS email FROM public.judicial_email_regra_usuarios WHERE regra_id = $1::bigint AND ativo = TRUE ORDER BY usuario_nome ASC`, r.id)
      return { id: r.id, nome: r.nome, users: users.map((u) => ({ id: text(u.id), nome: text(u.nome), email: text(u.email) })) }
    }
  }
...
async function saveFiles(files: Attachment[], protocolo: string, demandaId?: string, emailId?: string) {
  ...
  for (const file of files || []) {
    ...
    const item = { id: id("email_att_"), name, url: rel, relativePath: rel, storedName: stored, mimeType: text(file.contentType) || "application/octet-stream", size: Number(file.size || buffer.length || 0), source: "email" }
    saved.push(item)
    if (demandaId) await prisma.$executeRawUnsafe(`INSERT INTO public.anexos (id, "demandaId", nome, tipo, tamanho, categoria, descricao, "criadoPor", "criadoPorNome", "createdAt", "arquivoNomeOriginal", "arquivoPath", "mimeType") VALUES ($1, $2, $3, $4, $5, 'oficio', $6, 'sistema-email', 'Integração de e-mail', NOW(), $3, $7, $4)`, id("anx_"), demandaId, name, item.mimeType, item.size, `Ofício recebido por e-mail${emailId ? ` #${emailId}` : ""}.`, rel).catch(() => undefined)
  }
  return saved
}
async function saveProcessed(p: { uid: string; messageId: string; subject: string; from: string; date: string; fields: any; rule: Rule; status: string; match?: Match | null; osId?: string | null; error?: string; metadata?: unknown; read?: boolean }) {
  const rows = await prisma.$queryRawUnsafe<Array<{ id: string }>>(`
    INSERT INTO public.judicial_email_processados (message_uid, message_id, assunto, remetente, recebido_em, pge_net, processo, detectado_em, classificador, regra_id, regra_nome, status, monitoramento_id, demanda_id, os_id, erro, raw_metadata, processado_em, lido_em, created_at, updated_at)
    VALUES ($1,$2,$3,$4,$5::timestamptz,$6,$7,$8,$9,$10::bigint,$11,$12,$13::bigint,$14,$15::bigint,$16,$17::jsonb,NOW(),CASE WHEN $18::boolean THEN NOW() ELSE NULL END,NOW(),NOW())
    ON CONFLICT (message_id) WHERE message_id IS NOT NULL AND message_id <> '' DO UPDATE SET status=EXCLUDED.status, monitoramento_id=EXCLUDED.monitoramento_id, demanda_id=EXCLUDED.demanda_id, os_id=EXCLUDED.os_id, erro=EXCLUDED.erro, raw_metadata=EXCLUDED.raw_metadata, lido_em=COALESCE(EXCLUDED.lido_em, public.judicial_email_processados.lido_em), updated_at=NOW()
    RETURNING id::text AS id
  `, p.uid, p.messageId, p.subject, p.from, p.date, p.fields.pgeNet || null, p.fields.processo || null, p.fields.detectedIn || null, p.fields.classifier || null, p.rule.id || null, p.rule.nome || null, p.status, p.match?.monitoramentoId || null, p.match?.demandaId || null, p.osId || null, p.error || null, JSON.stringify(p.metadata || {}), Boolean(p.read))
  return rows[0]?.id || ""
}
...
async function deleteMsg(client: any, uid: unknown) {
  await client.messageFlagsAdd(String(uid), ["\\Seen", "\\Deleted"], { uid: true }).catch(() => undefined)
  await client.mailboxClose?.().catch?.(() => undefined)
}

export async function processUnreadEmailTriageV2(limit = 10) {
  ...
  try {
    ...
    try {
      ...
      const start = Math.max(1, total - Math.max(safeLimit * 4, 25) + 1)
      for await (const msg of client.fetch(`${start}:*`, { envelope: true, source: true, uid: true, flags: true }, { uid: false })) {
        if (items.length >= safeLimit) break
        const uid = text(msg.uid)
        let mail: Mail
        try { mail = (await simpleParser(msg.source as Buffer)) as Mail } catch (e) {
          const subject = text(msg.envelope?.subject) || "E-mail sem assunto"
          await saveProcessed({ uid, messageId: `${config.user}:${uid}`, subject, from: "", date: new Date().toISOString(), fields: {}, rule: { id: null, nome: "Erro de leitura", users: [] }, status: "ERRO_LEITURA", error: e instanceof Error ? e.message : String(e), read: true })
        ...
        if (old && ["VINCULADO", "OS_CRIADA", "ERRO_LEITURA"].includes(text(old.status).toUpperCase())) { await markSeen(client, uid); continue }
        ...
        try {
          const emailId = await saveProcessed({ uid, messageId, subject, from, date, fields, rule, status: "EM_PROCESSAMENTO", match, metadata: { attachments: files.map((f) => ({ filename: f.filename, contentType: f.contentType, size: f.size })) } })
          if (match?.monitoramentoId) {
            const saved = await saveFiles(files, match.protocolo || fields.processo, match.demandaId, emailId)
            await prisma.$executeRawUnsafe(`INSERT INTO public.judicial_movimentacoes (id, monitoramento_id, demanda_id, type, description, attachments, created_by, created_by_name, created_at) VALUES ($1, $2::bigint, $3, 'monitoramento', $4, $5::jsonb, 'sistema-email', 'Integração de e-mail', NOW())`, `jmov_email_${randomUUID()}`, match.monitoramentoId, match.demandaId || null, [`E-MAIL RECEBIDO AUTOMATICAMENTE`, `Assunto: ${subject}`, `Remetente: ${from}`, `PGE.net: ${fields.pgeNet || ""}`, `Anexos: ${saved.map((s) => s.name).join(" | ") || "nenhum"}`].join("\n"), JSON.stringify(saved))
